...
%----------------------------------------------------------------------------
%----------------------------------------------------------------------------
% Rotinas e funções do elemento Quadrilateral Quadrático Q8 

 % [var1,var2] = ElemQuadrilateralq8mindlin(comando,coordsElem,dadosElem,DElem)
 % inputs 
...
function [var1,var2] = ElemQuadrilateralq8mindlin(comando,coordsElem,dadosElem,DElem)
  ...
  switch(comando)
      ...
      otherwise 
          disp('ElemQuadrilateralq8mindlin: comando inválido')
  end
return
...
numNosElem = 8; %Número de nós do elemento

numGDLElem = 24; %Número de graus de liberdade do elemento 

% obtem a matriz constitutiva do elemento 
C = matrizconstitutivaq8mindlin(E,t,nu);
...
ordInt = dadosElem(4);  % ordem de integração numérica 
numNosElem = 8;   % Numero de nós do elemento 
...
function C = matrizconstitutivaq8mindlin(E,t,nu)
...
% obtem as funções de forma do elemento e monta a matriz N 
function N = Calcmatrizfuncforma(csi,eta)
  
   N8= 1/2*(1-csi)*(1-eta^2);
   N7= 1/2*(1-csi^2)*(1+eta);
   N6= 1/2*(1+csi)*(1-eta^2);
   N5= 1/2*(1-csi^2)*(1-eta);
   N4= 1/4*(1-csi)*(1+eta)*(-1-csi+eta);
   N3= 1/4*(1+csi)*(1+eta)*(-1+csi+eta);
   N2= 1/4*(1+csi)*(1-eta)*(-1+csi-eta);
   N1= 1/4*(1-csi)*(1-eta)*(-1-csi-eta);
 ...
 function  [dNdcsi, dNdeta] = Derivfuncformnatural(csi, eta)
  ...
  %de duas matrizes 1x8 ( 1 linha e 8 colunas ) 

  dNdcsi(1,1)=((-(2*eta)+2)*csi+(eta-eta^2))/4;
  dNdeta(1,1)=(-csi^2+(1-2*eta)*csi+2*eta)/4;

  dNdcsi(1,2)=((-(2*eta)+2)*csi+(eta^2-eta))/4;
  dNdeta(1,2)=(-csi^2+(2*eta-1)*csi+2*eta)/4;

  dNdcsi(1,3)=((2*eta+2)*csi+eta^2+eta)/4;
  dNdeta(1,3)=(csi^2+(2*eta+1)*csi+2*eta)/4;

  dNdcsi(1,4)=((2*eta+2)*csi-(eta^2+eta))/4;
  dNdeta(1,4)=(csi^2-(2*eta+1)*csi+2*eta)/4;

  dNdcsi(1,5)= (eta-1)*csi;
  dNdeta(1,5)=(csi^2-1)/2;

  dNdcsi(1,6)=(-eta^2+1)/2;
  dNdeta(1,6)= -(eta*csi)-eta;

  dNdcsi(1,7)=(-eta-1)*csi;
  dNdeta(1,7)=(-csi^2+1)/2;

  dNdcsi(1,8)=(eta^2-1)/2;
  dNdeta(1,8)=eta*csi-eta;
...
function  [dNdx, dNdy] = Derivfuncformcart(jac,dNdeta,dNdcsi)
 ...
 %inicializa a matriz 2x8 que conterá as derivadas cartesianas das funções de forma 



 %monta a matriz 2x8 de derivadas das funções de forma em coordenadas naturais 
